...
public class Notas extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        txtDniEst.addKeyListener(new java.awt.event.KeyAdapter() {
            public void keyTyped(java.awt.event.KeyEvent evt) {
                txtDniEstKeyTyped(evt);
            }
        });
    ...
    private void jButton1MousePressed(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_jButton1MousePressed
        ...
        if (duenio.buscarEstPorDni(txtDniEst.getText()) && validarCamposVacios() && validarNumeros(txtCalificacion) && validarNumeros(txtDniEst)) {
            int dni = Integer.parseInt(txtDniEst.getText());
            int calif = Integer.parseInt(txtCalificacion.getText());
            Nota mascota = new Nota(dni, txtCurso.getText(), calif, txtCondicion.getText());
    ...
    private void txtCalificacionKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtCalificacionKeyTyped
        ...
        // Verificar si la tecla presionada es un número o una tecla de borrar
        boolean num = Character.isDigit(key);
        boolean esBorrar = key == KeyEvent.VK_DELETE || key == KeyEvent.VK_BACK_SPACE;

        // Convertir el texto actual a un número para verificar el rango
        String textoActual = txtCalificacion.getText() + key;
        int valorIngresado = 0;
        try {
            valorIngresado = Integer.parseInt(textoActual);
        } catch (NumberFormatException e) {
            // Si el texto no se puede convertir a un número, no hacer nada
        }
        ...
        if (!num && !esBorrar) {
            ...
            evt.consume();
        } else if (valorIngresado < 0 || valorIngresado > 20) {
            Toolkit.getDefaultToolkit().beep();
            JOptionPane.showMessageDialog(null, "Por favor, ingrese un número entre 0 y 20.");
            evt.consume();
        }

        if (txtCalificacion.getText().trim().length() == 2) {
            Toolkit.getDefaultToolkit().beep();
            JOptionPane.showMessageDialog(null, "Por favor, ingrese un número de rango vigesimal");
            evt.consume();
        }

    }//GEN-LAST:event_txtCalificacionKeyTyped

    private void txtDniEstKeyTyped(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_txtDniEstKeyTyped
        // TODO add your handling code here:
        char key = evt.getKeyChar();
        // Verificar si la tecla presionada es un número o una tecla de borrar
        boolean num = Character.isDigit(key);
        ...
        if (txtDniEst.getText().trim().length() == 8) {
            Toolkit.getDefaultToolkit().beep();
            JOptionPane.showMessageDialog(null, "Por favor, ingrese un número de rango 8 digitos");
            evt.consume();
        }
    }//GEN-LAST:event_txtDniEstKeyTyped
    /**/
    ...
    private void cargarDatos() {
        ...
        Logica estu = new Logica();
        List<Nota> listaEstu = leer.leerNota();

        for (Nota estud : listaEstu) {
            String dniEstStr = String.valueOf(estud.getDniEst());
            String nombreEst = estu.buscarNombreEstPorDni(dniEstStr);
            modelNotas.addRow(new Object[]{nombreEst, estud.getCurso(), estud.getCalificacion(), estud.getCondicion()});
        }
    }
